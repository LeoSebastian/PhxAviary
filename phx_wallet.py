...
class PhxWallet(object):
    ...
    def __init__(self, priv_key, pub_key, provider, min_reinvest):
        self.priv_key = priv_key
        self.pub_key = pub_key
        self.web3_obj = Web3(HTTPProvider(provider))
        self.min_reinvest = self.web3_obj.toWei(min_reinvest, 'ether')
        self.ethphoenix = self.web3_obj.eth.contract(address='0x2Fa0ac498D01632f959D3C18E38f4390B005e200',
            abi=custom_abi)
        self.phoenixcoin = self.web3_obj.eth.contract(address='0x14b759A158879B133710f4059d32565b4a66140C',
            abi=custom_abi)

        ### check that the wallet has EPX
        epx_bal = self.ethphoenix.functions.tokenBalance(pub_key).call()
        if (epx_bal == 0):
            sys.exit('This wallet has no EPX: %s' % pub_key)
